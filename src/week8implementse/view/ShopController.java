...
public class ShopController {
     ...
     * </pre>
     *
     * @param b The Model with all of the back-end links that the store is to
     * use
     */
     ...
     */
    public User getCurrentCustomerDetails() {
     ...
     */
    public void updateUserDetails(User c) {
    ...
    public void addToCart(Product p, float quantity) {
        //往购物车中添加quantity数量的product
        cart.add(p, quantity);
        //需要从总的productlist里面,减去该product的相应quantity数量
    }
    ...
    public void attemptTransaction() {
        User c = getBackend().getUserInfo(currentUserID);
        String prefix = "Order failed! ";
        if (c.name.trim().equals("")) {
            showPopup(prefix + "You have not entered your full name!");
            return;
        } else if (c.username.trim().equals("")) {
            showPopup(prefix + "You have not entered your user name!");
            return;
        } else if (c.password.trim().equals("")) {
            showPopup(prefix + "You have not entered your password!");
            return;
        } else if (c.verificationQuestion.trim().equals("")) {
            showPopup(prefix + "You have not entered your verificationQuestion!");
            return;
        } else if (c.answer.trim().equals("")) {
            showPopup(prefix + "You have not entered your answer!");
            return;
        } else if (c.dob.trim().equals("")) {
            showPopup(prefix + "You have not entered your birthday!");
            return;
        }
        ...
        if (!success) {
            ...
            readOrWrite.savePurchaseRecord(cart,currentUserID);
            this.cart.clear();
            this.showCartView();
        }
    }
